...
/** Generates a substantial fictional POI catalog without shipping a giant hand-authored file. */
...
for (let i = 0; i < 280; i += 1) {
  ...
  const name = `${names[category][i % names[category].length]} ${String(i + 1).padStart(2, '0')}`;
