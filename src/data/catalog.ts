...
for (let i = 0; i < 320; i += 1) {
  ...
  catalogLocations.push({ id: `catalog-${i}`, name: `${names[category][i % names[category].length]} ${String(i + 1).padStart(2, '0')}`, category, district: district.name, rating: (4 + ((i * 13) % 10) / 10).toFixed(1), hours: i % 11 === 0 ? 'Always open' : i % 5 === 0 ? '18:00 — 02:00' : '08:00 — 21:00', description: `A fictional ${category.toLowerCase()} stop in ${district.name}, with its own corner of Saint Meridian atmosphere.`, address: `${district.code} ${['Avenue', 'Street', 'Boulevard', 'Lane'][i % 4]} ${10 + ((i * 17) % 890)}`, x: district.x + ((i * 13) % 17 - 8) * 0.72, z: district.z + ((i * 19) % 15 - 7) * 0.7, accent: accents[i % accents.length] });
}
