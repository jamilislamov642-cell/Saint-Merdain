...
export function createCity(onSelect: SelectHandler) {
  ...
  const dayBackground = new THREE.Color('#0b1113');
  const nightBackground = new THREE.Color('#070b14');
  scene.background = dayBackground.clone();
  scene.fog = new THREE.Fog(dayBackground, 62, 190);
  ...
  const addRoad=(x:number,z:number,w:number,d:number,rot=0)=>{const r=new THREE.Mesh(new THREE.BoxGeometry(w,.08,d),roadMat);r.position.set(x,.04,z);r.rotation.y=rot;city.add(r);const l=new THREE.Mesh(new THREE.BoxGeometry(w*.96,.012,.035),laneMat);l.position.set(x,.1,z);l.rotation.y=rot;city.add(l);};
  for(let z=-43;z<=43;z+=8)addRoad(0,z,138,2.1); for(let x=-58;x<=52;x+=8)addRoad(x,0,2.1,90,Math.PI/2); addRoad(-34,-1,2.8,90); addRoad(-10,0,3,90); addRoad(25,0,3.2,94,Math.PI/2); addRoad(-8,30,3,70,Math.PI/2);
  districts.forEach((district,di)=>{addDistrictOverlay(city,district);const count=di===3?52:20+(di%5)*5;for(let i=0;i<count;i++){const a=i*2.399+di,r=2+(i%7)*1.18,x=district.x+Math.cos(a)*r,z=district.z+Math.sin(a)*r,w=1.1+(i%5)*.4,h=(di===3?4:1.2)+(i%8)*.6;const b=new THREE.Mesh(new THREE.BoxGeometry(w,h,1.1+(i%4)*.4),mat(palette[(i+di)%palette.length]));b.position.set(x,h/2,z);city.add(b);}});
  ...
  addLandmark(city,-38,-21,'Lighthouse','#e1c77b'); addLandmark(city,35,-29,'Airport','#87b5c8'); addLandmark(city,-15,20,'Terminal','#c9a767'); addLandmark(city,30,-16,'Observatory','#d58b6d'); addLandmark(city,8,28,'Civic Hall','#9bb57e');
  ...
  catalogLocations.forEach(loc=>{const pin=new THREE.Group();pin.position.set(loc.x,.65,loc.z);pin.userData.location=loc;const stem=new THREE.Mesh(new THREE.CylinderGeometry(.035,.035,.8,6),new THREE.MeshBasicMaterial({color:loc.accent}));stem.position.y=.4;pin.add(stem);const dot=new THREE.Mesh(new THREE.SphereGeometry(.16,8,6),new THREE.MeshBasicMaterial({color:loc.accent}));dot.position.y=.85;pin.add(dot);markerGroup.add(pin);markerById.set(loc.id,pin);});
  const routeGroup=new THREE.Group();routeGroup.name='route';city.add(routeGroup);
  const drawRoute=(route:RouteResult|null)=>{routeGroup.clear();if(!route)return;const points=route.nodes.map(node=>new THREE.Vector3(node.x,.34,node.z));if(points.length<2)return;const curve=new THREE.CatmullRomCurve3(points);const geometry=new THREE.BufferGeometry().setFromPoints(curve.getPoints(Math.max(20,points.length*8)));routeGroup.add(new THREE.Line(geometry,new THREE.LineBasicMaterial({color:'#d9b36e'})));};
  ...
  const renderer=new THREE.WebGLRenderer({antialias:true,powerPreference:'high-performance'});renderer.setPixelRatio(Math.min(window.devicePixelRatio,2));renderer.shadowMap.enabled=true;renderer.toneMapping=THREE.ACESFilmicToneMapping;renderer.toneMappingExposure=1.1;
  ...
  const setNight=(night:boolean)=>{const background=night?nightBackground:dayBackground;scene.background=background.clone();(scene.fog as THREE.Fog).color.copy(background);sun.color.set(night?'#5d739f':'#e1c08c');sun.intensity=night?.8:2.2;ambient.intensity=night?.6:1.2;};
